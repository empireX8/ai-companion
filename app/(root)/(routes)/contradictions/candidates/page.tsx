...
export const CANDIDATE_REVIEW_BACK_HREF = "/your-map";
export const CANDIDATE_REVIEW_BACK_LABEL = "Back to Map";

export async function confirmCandidate(
  id: string,
  fetchImpl: typeof fetch = fetch,
): Promise<void> {
  const res = await fetchImpl(`/api/contradiction/${id}`, {
    method: "PATCH",
...
export async function dismissCandidate(
  ...
  const res = await fetchImpl(`/api/contradiction/${id}`, { method: "DELETE" });
...
export function CandidateReviewContent({
  items,
  loading,
  error,
  busy,
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto">
      <div className="mx-auto w-full max-w-2xl px-6 py-8">
        <Link
          href={CANDIDATE_REVIEW_BACK_HREF}
          className="mb-6 inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground"
        >
          {CANDIDATE_REVIEW_BACK_LABEL}
        </Link>
