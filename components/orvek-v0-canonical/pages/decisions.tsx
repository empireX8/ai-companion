          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {[
              ...
              <button
                ...
                type="button"
                onClick={() => {
                  if (a.label === "Review due decision") {
                    const dueId =
                      decisionListGroups.find((g) => /due|outcome/i.test(g.heading))
                        ?.ids[0] ?? decisionListGroups[0]?.ids[0]
                    if (dueId) openDecision(dueId)
                  }
                  if (a.label === "Add outcome" && workspaceId) {
        ...
        <div className="o-sunken m-3 mt-0 min-h-0 overflow-y-auto rounded-2xl px-3 py-4 lg:mr-1.5">
          {decisionListGroups.length === 0 ? (
            <p className="px-2 py-6 text-[13px] text-muted-foreground">
              No decisions in your model yet.
            </p>
          ) : (
            decisionListGroups.map((group) => (
        ...
        <div className="min-h-0 overflow-y-auto px-6 py-6 lg:px-8">
          <div className="mx-auto max-w-2xl">
            {!decision ? (
              <div className="py-16 text-center">
                ...
                <p className="mt-1.5 text-[13px] text-muted-foreground">
                  Enter a decision or select one from the list when evidence exists.
                </p>
              </div>
            ) : (
              <>
            ...
            <div className="mt-5 flex flex-wrap gap-1.5">
              ...
              <button
                type="button"
                onClick={() => openReport(decision.id)}
                className="o-calm inline-flex items-center gap-1.5 rounded-[8px] bg-secondary/70 px-2.5 py-1.5 text-xs font-medium text-foreground hover:bg-accent/60"
              >
