...
export function DecisionsPage() {
  ...
  const lists = isProduction
    ? [
        { heading: "Active", ids: decisionListGroups.find((g) => g.heading === "Active")?.ids ?? [] },
        { heading: "Chosen", ids: decisionListGroups.find((g) => g.heading === "Chosen")?.ids ?? [] },
        {
          heading: "Outcome due",
          ids: decisionListGroups.find((g) => g.heading === "Outcome due")?.ids ?? [],
          tone: "action" as const,
        },
        { heading: "Reviewed", ids: decisionListGroups.find((g) => g.heading === "Reviewed")?.ids ?? [] },
      ]
    : decisionListGroups.length > 0
      ? decisionListGroups
      : LISTS
            ...
            {isProduction ? (
              <>
                ...
                <span className="font-medium text-foreground">
                  {decisionsHeaderStats?.reviewed ?? 0}
                </span>{" "}
                reviewed
              </>
            ) : (
        ...
        <div className="rounded-2xl bg-evidence-muted/50 p-3 ring-1 ring-inset ring-primary/15">
          <div className="o-material flex items-center gap-2 rounded-[9px] px-3 py-2">
            ...
            <button
              ...
              className={cn(
                "inline-flex shrink-0 items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90",
          ...
          {lists.map((group) => (
            <div key={group.heading} className="mb-4">
              ...
              <div className="mt-1.5 space-y-0.5">
                {group.ids.length === 0 && isProduction ? (
                  <p className="px-2 py-1 text-[11px] text-muted-foreground">—</p>
                ) : null}
                {group.ids.map((id) => {
        ...
        <div className="min-h-0 overflow-y-auto px-6 py-6 lg:px-8">
          <div className="mx-auto max-w-2xl">
            ...
            <div className="mt-5 flex flex-wrap gap-1.5">
              ...
              <>
                <h2 className="text-2xl font-semibold leading-tight tracking-tight text-foreground text-balance">
                  {emptyCopyBySlot?.decisionsEmpty ??
                    "Select a decision from the list or capture a new one above."}
                </h2>
                <div className="mt-4 rounded-lg border-l-2 border-primary bg-evidence-muted/40 px-4 py-3">
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-primary">
                    Evidence-backed read
                  ...
                  <p className="mt-1 text-[15px] leading-relaxed text-foreground">
                    {emptyCopyBySlot?.decisionsProjectionEmpty ??
                      "Projection appears when a decision has enough linked evidence."}
                  </p>
                </div>
                <WSBlock label="Options">
                  <div className="o-material rounded-[10px] p-4 text-[13px] text-muted-foreground">
                    {emptyCopyBySlot?.decisionsOptionsEmpty ??
                      "Options appear when a decision is captured and compared."}
                  </div>
                </WSBlock>
                <WSBlock label="Constraints, wants & fears">
                  <div className="o-material rounded-[10px] p-4 text-[13px] text-muted-foreground">
                    {emptyCopyBySlot?.decisionsContextEmpty ??
                      "Context fields appear when linked background is available."}
                  </div>
                </WSBlock>
                <WSBlock label="Projection">
                ...
                <div className="o-material mt-5 rounded-[10px] p-4">
                  <SectionLabel>Outcome &amp; what it reveals</SectionLabel>
                  <p className="mt-1.5 text-[13px] text-muted-foreground">
                    {emptyCopyBySlot?.decisionsOutcomeEmpty ??
                      "Outcome review appears when a decision window closes."}
                  </p>
                </div>
              </>
            ) : (
