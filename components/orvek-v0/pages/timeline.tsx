...
export function TimelinePage() {
  const { timeline } = useOrvekData();
  const pageHandlers = useOrvekPageHandlers().timeline;

  if (!timeline || !pageHandlers) {
    return (
      <div className="px-6 py-6" data-testid="orvek-v0-timeline-page">
        <p className="text-sm text-muted-foreground">Loading…</p>
      </div>
    );
  }

  return <V0TimelineView data={timeline} handlers={pageHandlers} />;
}
