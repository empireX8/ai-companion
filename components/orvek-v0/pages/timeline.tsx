...
const TIMELINE_SHELL_GROUPS: { heading: string; ids: string[] }[] = [
  { heading: "Today", ids: [] },
  { heading: "This week", ids: [] },
  { heading: "Last week", ids: [] },
  { heading: "Earlier", ids: [] },
  { heading: "Imported history", ids: [] },
]
...
export function TimelinePage() {
  ...
  const groups = isProduction
    ? TIMELINE_SHELL_GROUPS.map((shell) => ({
        heading: shell.heading,
        ids: timelineGroups.find((group) => group.heading === shell.heading)?.ids ?? [],
      }))
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="px-6 pt-5 pb-4 lg:px-8">
        ...
        {isProduction && timelineIsLoading && (
          <p className="mt-2 text-[13px] text-muted-foreground">Loading timeline…</p>
        )}
      </div>
            ...
            <div className="relative pl-5">
              ...
              {groups.map((group) => {
                const visible = group.ids
                  .map((id) => getObject(id))
                  .filter((item): item is NonNullable<ReturnType<typeof getObject>> => {
                    if (!item) return false;
                    return (
                      matches(filter, item.eventType ?? item.reportType ?? "", item.tags ?? []) &&
                      (query === "" || item.title.toLowerCase().includes(query.toLowerCase()))
                    );
                  })
                if (visible.length === 0 && !isProduction) return null
                return (
                      ...
                      <div className="o-material rounded-[10px] p-4 text-[13px] text-muted-foreground">
                        {group.heading === "Today"
                          ? (emptyCopyBySlot?.timelineEmpty ??
                            "No published evolution appears in this window yet.")
                          : "—"}
                      </div>
                    )}
                  </div>
