# 12 — Tests and validation

## Constraint

No second live provider run during validation. Observed: **no second live run**.

## Pre-live focused deterministic gates

| Suite | Files | Tests | Result |
|-------|-------|-------|--------|
| CEQR-014 prompt repair | 1 | 12 | PASS |
| CEQR-013 diagnostic receipts | 1 | 7 | PASS |
| CEQR-012 semantic compatibility | 1 | 28 | PASS |
| CEQR-011 provider/referee | 1 | 49 | PASS |
| AI SDK runner options | 1 | 3 | PASS |
| **Combined** | **5** | **99** | **PASS** |

## Post-live focused

| Suite | Files | Tests | Result |
|-------|-------|-------|--------|
| CEQR-015 receipt/invariants | 1 | 7 | PASS |
| CEQR-014 / 013 / 012 / 011 / runner options | 5 | 99 | PASS |
| **Combined** | **6** | **106** | **PASS** |

## Related suites

12 files / **335** passed / 0 failed (CEQR-010 natural-entry, adjudication,
objectivity referee, confidence, repaired persistence, dual-source
presentation/routes, evidence, source, dual-side lineage, persistence plan,
duplicate-prevention schema).

## Full Vitest

| Metric | Baseline (after CEQR-014) | This slice |
|--------|---------------------------|------------|
| Failed files | 5 | 5 |
| Passed files | 319 | 320 (+1 CEQR-015) |
| Failed tests | 7 | 7 |
| Passed tests | 4345 | 4352 (+7 CEQR-015) |
| Failure set expanded | — | **NO** |

Known failing files unchanged:

- `lib/__tests__/canonical-fixture-composition-gate.test.ts`
- `lib/__tests__/evidence-pointer-surfacing-rationale-schema.test.ts`
- `lib/__tests__/orvek-adapters.test.ts`
- `lib/__tests__/surfaced-evidence-pointer-schema.test.ts`
- `lib/__tests__/today-production-movement-depth.test.ts`

## Other gates

| Gate | Result |
|------|--------|
| `npx tsc --noEmit` | pass |
| Changed-file ESLint | pass |
| Production build (env loaded) | pass |
| `git diff --check` | pass |
| Secret pattern scan (CEQR-015 additions) | pass |
| Real-account-identifier scan (CEQR-015 additions) | pass |
| Source-length metadata scan | pass |
| Provider-output-repair pattern scan | pass |
| Ordinary route/import/live-wiring scan | pass |
| Trust language / legacy surfaces | pass |
