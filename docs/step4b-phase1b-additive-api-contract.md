...
| POST | `/api/user-map/conclusions` | Explicit/manual creation path | `UserMapConclusion` | Create | Required | Body fields | `201 { item }` | N/A | field + enum validation + default visibility assignment | no auto synthesis; no public/internal ambiguity on visibility | Controlled/manual route defaults created rows to `user_visible`; `internal_only` creation reserved for restricted internal writers. **Legacy relative to production intelligence flow** (2026-06-09 audit-wave synthesis): production intelligence uses candidate creation → internal review → publish (`POST /api/internal/user-map/candidates/[id]/publish`); this POST path creates `user_visible` conclusions immediately without candidate lifecycle review. Safe from public API response projection after `2c54bce`; not equivalent to reviewed intelligence. Long-term status (keep, restrict, deprecate, or user-authored map entries) is an open product/governance decision — see `docs/engineering-ledger.md` Post-UserMap Audit Wave Synthesis Closeout |
