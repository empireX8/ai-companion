...
describe("contradiction detail page continuity", () => {
  it("uses public receipt href helper and avoids manual tension receipt templates", () => {
    const source = readFileSync(
      "app/(root)/(routes)/contradictions/[id]/page.tsx",
      "utf8"
    );
    ...
    expect(source).not.toContain("href={`/library/receipt-tension-${detail.id}`}");
  });
});
