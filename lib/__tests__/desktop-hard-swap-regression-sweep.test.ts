...
describe("desktop hard-swap regression sweep", () => {
  ...
  it("3 — Today remains store-driven re-entry surface inside canonical pages", () => {
    const todayPage = readSource("components/orvek-v0-canonical/pages/today.tsx");
    const workbench = readSource("components/orvek-v0-canonical/workbench.tsx");

    expect(workbench).toContain('case "today"');
    expect(todayPage).toContain("useWorkbench");
    expect(todayPage).toContain("select");
  ...
  it("4 — Map bridge stays readiness-gated in hybrid workbench", () => {
    ...
    const mapPage = readSource("components/orvek-v0-canonical/pages/map.tsx");
    const hybridHook = readSource(
      "components/orvek-workbench/useOrvekHybridWorkbenchDataApi.ts",
    );
    ...
    expect(mapPage).toContain("useCanonicalData");
  ...
  it("5 — Timeline remains semantic evolution surface, not Calendar", () => {
    const timelinePage = readSource("components/orvek-v0-canonical/pages/timeline.tsx");
    const hybridApi = readSource("lib/orvek-v0/production/hybrid-workbench-api.ts");

    expect(timelinePage).toContain("timelineFilters");
  ...
  it("6 — Decisions bridge stays readiness-gated", () => {
    const hybridApi = readSource("lib/orvek-v0/production/hybrid-workbench-api.ts");
    const decisionsPage = readSource("components/orvek-v0-canonical/pages/decisions.tsx");
  ...
  it("9 — legacy route-first pages remain quarantined; active canonical pages use store navigation", () => {
    ...
    for (const pagePath of ACTIVE_CANONICAL_PAGES) {
