...
describe("bounded experiment hybrid fetch bridge", () => {
  ...
  it("FieldworkBridge rendering path is unchanged", () => {
    ...
    expect(explorePageSource).toContain('onClick={() => !isProduction && onSelect("f2")}');
    expect(explorePageSource).not.toContain("exploreFieldworkIds");
