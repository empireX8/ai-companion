/**
 * Production-neutral contradiction natural-entry orchestrator.
 *
 * Shared pipeline (not a proof identity):
 ...
 *
 * Callers inject StructuredModelRunner / ObjectivityReferee / DB boundaries.
 * Does not construct live providers. Does not expose authorised WeakSet plans.
 */
...
import {
  ...
  type SameSessionSelectionRefereeMode,
...
export const CONTRADICTION_NATURAL_ENTRY_VERSION =
  "contradiction-natural-entry-v1" as const;

export type ContradictionNaturalEntryOutcome =
  | "created"
  | "reused"
  | "no_candidate"
  | "routed_elsewhere"
...
export type ContradictionNaturalEntryGateStage =
  | "selection"
  | "lineage"
  | "confidence"
  | "persistence_plan"
...
export type ContradictionNaturalEntryPresentationStatus =
  | "not_requested"
  | "resolved"
  | "unavailable"
  | "failed";

export type ContradictionNaturalEntryMessageResolver = {
  /**
...
export type ContradictionNaturalEntryInput = {
  userId: string;
  sessionId: string;
  ...
  /**
   * Hard cap on adjudicator candidates (production default: 3).
   * Omitted → existing unbounded same-session pool behaviour.
   */
  maxAdjudicatorCandidates?: number;
  /**
   * `per_class_a` — referee during each Class A adjudication (proof default).
   * `sole_winner_only` — at most one referee call after zero-or-one selection.
   */
...
export type ContradictionNaturalEntryResult = {
  pipelineVersion: typeof CONTRADICTION_NATURAL_ENTRY_VERSION;
  outcome: ContradictionNaturalEntryOutcome;
   ...
   */
  gateStoppedAt: ContradictionNaturalEntryGateStage | null;
  ...
  presentationStatus: ContradictionNaturalEntryPresentationStatus;
...
function baseResult(
  partial: Omit<ContradictionNaturalEntryResult, "pipelineVersion">,
): ContradictionNaturalEntryResult {
  return {
    pipelineVersion: CONTRADICTION_NATURAL_ENTRY_VERSION,
    ...partial,
  };
}
...
function presentationDefaults(
  status: ContradictionNaturalEntryPresentationStatus,
): {
...
function validateCurrentMessage(
  ...
  if (currentMessage.sessionId !== sessionId) {
    return {
      ok: false,
      message: "currentMessage.sessionId does not match the sessionId.",
    };
  }
  return { ok: true };
}
...
/**
 * Execute the contradiction natural-entry pipeline.
 *
 ...
 */
export async function runContradictionNaturalEntry(
  input: ContradictionNaturalEntryInput,
): Promise<ContradictionNaturalEntryResult> {
  ...
  if (!confidenceResult.ok || !confidenceResult.continuationReady) {
    ...
    let outcome: ContradictionNaturalEntryOutcome = "no_candidate";
  ...
  let presentationStatus: ContradictionNaturalEntryPresentationStatus =
    "not_requested";
