...
export class ContradictionSourceError extends Error {
  status: number;
  code: string;
...
type ResolveContradictionSourceParams = {
  userId: string;
  sourceSessionId?: string;
  sourceMessageId?: string;
  db: ContradictionSourceDb;
};

export async function resolveContradictionSource({
  userId,
  sourceSessionId = "",
  sourceMessageId = "",
  db,
}: ResolveContradictionSourceParams): Promise<{
  sourceSessionId: string;
  sourceMessageId: string;
}> {
  ...
  if (effectiveSourceMessageId) {
    ...
    if (!sourceMessage) {
      throw new ContradictionSourceError(
        404,
  ...
  if (
    ...
    sourceMessage.sessionId !== effectiveSourceSessionId
  ) {
    throw new ContradictionSourceError(
      400,
